-- Lock rental payment/state integrity after the approve->pay path was exposed.
-- Authenticated clients may only perform owner-driven status transitions:
--   pending -> approved/cancelled, paid -> active.
-- Money totals, Stripe intent IDs, contract flags, and paid/completed states are
-- written by service-role Edge Functions after Stripe verification.
