...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Verify rental belongs to this user and can still be paid.
    ...
    const amount = toStripeCents(rental.total_price);
    const deposit = toStripeCents(rental.deposit_amount ?? 0);
    if (amount <= 0) return json({ error: "Rental total must be positive" }, 400);
...
function toStripeCents(amount: number | string | null): number {
