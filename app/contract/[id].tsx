...
function ContractPaySection({ rental, agreed, user, router }: any) {
  ...
  async function handleAgreeAndPay() {
    ...
    try {
      ...
      await authorizePaymentSheet(intentBody.paymentIntentClientSecret, "Twirl");
      if (intentBody.depositIntentClientSecret) {
        await authorizePaymentSheet(intentBody.depositIntentClientSecret, "Twirl security deposit");
      }
  ...
  async function authorizePaymentSheet(paymentIntentClientSecret: string, merchantDisplayName: string) {
